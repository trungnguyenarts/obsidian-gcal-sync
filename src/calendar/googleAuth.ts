...
export class GoogleAuthManager {
    ...
    constructor(plugin: GoogleCalendarSyncPlugin) {
        ...
        this.clientId = this.plugin.settings.customClientId || credentials.clientId;
        this.clientSecret = this.plugin.settings.customClientSecret || credentials.clientSecret || null;
    ...
    private async exchangeCodeForTokens(code: string): Promise<OAuth2Tokens> {
        if (this.clientSecret) {
            ...
            try {
                ...
                if (response.status >= 400) {
                    console.error('❌ Google token exchange failed:', response.status, response.text);
    ...
    async refreshAccessToken(): Promise<OAuth2Tokens> {
        ...
        if (this.clientSecret) {
            ...
            try {
                ...
                if (response.status >= 400) {
                    console.error('Direct token refresh failed:', response.status, response.text);
                    throw new Error(`Direct token refresh failed with status ${response.status}`);
                }

                const tokens: OAuth2Tokens = {
                    access_token: response.json.access_token,
                    refresh_token: this.refreshToken, // Google might not return a new refresh token
                    scope: response.json.scope,
                    token_type: response.json.token_type,
                    expiry_date: Date.now() + response.json.expires_in * 1000
                };
